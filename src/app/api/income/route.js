...
export async function GET(req) {
  try {
    ...
    const incomeSources = await IncomeSource.find({ userId })
      .sort({ createdAt: -1 })
      .lean();
    const clients = await Client.find({ userId }).select("name company").lean();

    const updatedIncome = incomeSources.map((src) => {
      const client = clients.find((c) => c._id.toString() === src.clientId?.toString()) || null;
      return {
        ...src,
        clientName: src.clientName || (client?.company ? `${client.company} — ${client.name}` : client?.name || "Unknown"),
...
export async function POST(req) {
  try {
    ...
    if (body.clientId) {
      ...
      if (client) {
        clientName = client.company ? `${client.company} — ${client.name}` : client.name;
      }
    }

    // Ensure type is valid
    const type = validTypes.includes(body.type?.trim()) ? body.type.trim() : "Fixed Salary";
...
export async function PUT(req) {
  try {
    ...
    if (body.type) body.type = validTypes.includes(body.type?.trim()) ? body.type.trim() : "Fixed Salary";

    if (body.clientId) {
      const client = await Client.findById(body.clientId).lean();
      if (client) body.clientName = client.company ? `${client.company} — ${client.name}` : client.name;
    }

    const updated = await IncomeSource.findOneAndUpdate({ _id: body._id, userId }, body, { new: true, runValidators: true });
...
export async function DELETE(req) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "Income source ID required" }, { status: 400 });

    const deleted = await IncomeSource.findOneAndDelete({ _id: id, userId });
    if (!deleted) return NextResponse.json({ error: "Income source not found" }, { status: 404 });
